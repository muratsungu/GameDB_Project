...
# ----------------------------
# INCREMENTAL FETCH WITH UPDATED + DATES
# ----------------------------
def fetch_incremental():
    ...
    page = 1
    while True:
        # updated varsa öncelik, yoksa fallback olarak dates
        params = {
            "key": RAWG_API_KEY,
            "page": page,
            "page_size": PAGE_SIZE,
            "updated": f"{last_run},{now_iso}",
            "dates": f"{last_run},{now_iso}",
        }
        ...
        if not data.get("next"):
            break
        page += 1
        time.sleep(0.5)  # throttle, API overload olmasın
